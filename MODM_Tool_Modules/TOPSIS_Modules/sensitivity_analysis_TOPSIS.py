...
def ridgelineplot_sensitivity_results(scores_df, user_input, directory):
    ...
    if user_input == 'yes':
        plt.savefig(os.path.join(directory, 'TOPSIS_ridgeline_plot.png'), dpi=500)
    plt.show()
...
def plot_bar_chart_topsis_top1(scores, user_input, directory):
    ...
    if user_input == 'yes':
        ...
        plt.savefig(
            os.path.join(
                directory,
                'TOPSIS_top1_%_barplot.png'),
            dpi=500,
            bbox_inches='tight')
    plt.show()


def plot_bar_chart_topsis_top3(scores, user_input, directory):
    # Sort scores for better visualization
    sorted_scores = scores.sort_values(ascending=False)
...
def sensitivity_analysis(
    ...
    plot_bar_chart_topsis_top1(filtered_top_serie, user_input, directory)
    plot_bar_chart_topsis_top3(filtered_top3_serie, user_input, directory)
